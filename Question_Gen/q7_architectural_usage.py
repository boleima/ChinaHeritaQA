"""Generate question q7: Which architectural-usage description correctly matches this image?"""
import os
import json
import random
...
if __name__ == "__main__":
    ...
    Question: Which description of the architectural usage is correct for this image?
    Options list: [
        A: correct answer,
    ...
    save_name = config.QUESTION_FILES["q7"]
    rt_img_weibo = config.IMG_PREFIX_CHINA

    heritage_meta = data_filter(heritage_meta, ["Number of Images"], 0, False)
    heritage_meta = data_filter(heritage_meta, ['BUilding_use','cn'], 'none', False)
    world_heritage_info = data_filter(world_heritage_info, ['BUilding_use','cn'], 'none', False)
    ...
    for key, val in tqdm(heritage_meta.items()):
        Building_use = val['BUilding_use']
        img_url_list = val['Image URLs']
        option_A = generate_option(cn_text=Building_use['cn'], en_text=Building_use['en'], is_true=True)
        for img_url in img_url_list:
            ...
            option_C_text = random_exclude(tmp_keys_infos, [Building_use['cn']], ['BUilding_use','cn'])
            ...
            if len([k for k in type_keys if heritage_meta[k]['BUilding_use']['cn'] not in (Building_use['cn'], option_C_text['cn'])]) == 0:
                type_keys = all_keys
            tmp_keys_infos = map_heritage_infos_by_keys(heritage_meta, type_keys)
            option_B_text = random_exclude(tmp_keys_infos, [Building_use['cn'], option_C_text['cn']], ['BUilding_use','cn'])
            option_B = generate_option(cn_text=option_B_text['cn'], en_text=option_B_text['en'])

            # option_D: any Chinese heritage site
            option_D_text = random_exclude(heritage_meta, [Building_use['cn'], option_C_text['cn'], option_B_text['cn']], ['BUilding_use','cn'])
            ...
            option_E_text = random_exclude(type_worlds, [Building_use['cn'], option_C_text['cn'], option_B_text['cn'], option_D_text['cn']], ['BUilding_use','cn'])
    ...
    question_one_info['question_info']={
        ...
        "question_content":{'cn':'关于该图片建筑用途介绍正确的是?','en':'Which description of the architectural usage of this picture is correct?'},
