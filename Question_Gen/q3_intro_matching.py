"""Generate question q3 (intro_matching): Which brief introduction correctly describes this image?"""
import os
import json
import random
...
import config
from utils.data_preprocess import get_province_city, random_exclude, openJson, generate_option


if __name__ == "__main__":
    '''
    Generate single-choice questions.
    Question: Which brief introduction correctly describes this image?
    Options list: [
        A: correct answer,
        B: same-type heritage site Brief_intro,
        C: same-province heritage site Brief_intro,
        D: any Chinese heritage site Brief_intro,
        E: non-Chinese heritage site Brief_intro
    ]
    '''
    heritage_meta = openJson(config.HERITAGE_BRIEF_INTRO)
    ...
    save_name = config.QUESTION_FILES["q3"]
    ...
    for key, val in tqdm(heritage_meta.items()):
        Brief_intro = val['Brief_intro']
        img_url_list = val['Image URLs']
        option_A = generate_option(cn_text=Brief_intro['cn'], en_text=Brief_intro['en'], is_true=True)
        ...
        for img_url in img_url_list:
            ...
            if city_str != "":
                ...
                candidate_lit = [k for k in pro_heritage_file[pro] if k in heritage_meta]
            ...
            option_C_intro = heritage_meta[option_C_key]['Brief_intro']
            option_C = generate_option(cn_text=option_C_intro['cn'], en_text=option_C_intro['en'])
            ...
            option_B_intro = heritage_meta[option_B_key]['Brief_intro']
            option_B = generate_option(cn_text=option_B_intro['cn'], en_text=option_B_intro['en'])

            # option_D: any Chinese heritage site
            option_D_key = random_exclude(all_keys, [key, option_B_key, option_C_key])
            option_D_intro = heritage_meta[option_D_key]['Brief_intro']
            option_D = generate_option(cn_text=option_D_intro['cn'], en_text=option_D_intro['en'])

            # option_E: non-Chinese heritage site
            option_E_key = random_exclude(list(world_heritage_info.keys()), [])
            option_E = generate_option(cn_text=world_heritage_info[option_E_key]['Brief_intro']['cn'],
                                       en_text=world_heritage_info[option_E_key]['Brief_intro']['en'])
    ...
    question_one_info['question_info'] = {
        ...
        "question_content": {
            'cn': '关于该图片简要介绍正确的是?',
            'en': 'Which brief introduction regarding this picture is correct?'
        },
    ...
    print(f"Saved {idx} questions to {save_name}")
